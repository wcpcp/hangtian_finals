...
def traffic(img):
    ...
    for i , (mask, color) in enumerate(zip(masks, colors)):
        ...
        for contour in contours:
            ...
            if max_area > 50 and max_area < 1000:
                print(max_area)
                cv2.circle(img, (cx, cy), 5, color, -1)
                color_flag = selected_mask_index + 1
...
def PoseCallBack(msg):
    ...
    onlyone = rospy.get_param("/onlyone")

    if  y <= -4.95 and onlyone == 1:
        rospy.set_param("/trafficlight_enable", 1)
        onlyone=0
        rospy.set_param("/onlyone", 0)
    # else:
        # rospy.set_param("/trafficlight_enable", 0) 
    ...
    def line_follow(self,img_bgr):
        ...
        if  vision_enable:
            ...
            if self.center_point is not None:
                ...
                print ("find out, center point: ", self.center_point)
            else:
                print ("not found")
...
def is_find_word(image):
    global ocr_handle
    results = ocr_handle.text_predict(frame, short_size=960)
