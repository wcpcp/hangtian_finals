...
{
    // printf("doPose回调函数");
    if(p->pose.pose.position.x  < -1.1  && p->pose.pose.position.y > -1.70842505593  && p->pose.pose.position.y <-1.1)
    //if(  p->pose.pose.position.y > 1.68*p->pose.pose.position.x )
    {
        ros::param::set("/offset_enable",0); //关闭视觉寻线
        ROS_INFO("send start point  ");
        send_point( -0.274123140841, -0.0415280796032 ,0.0299248307134,0.999552151969 ); //路径规划回到起点
    }
}
...
int main(int argc, char** argv){
    ...
    ros::NodeHandle nh;
    ros::Subscriber sub = nh.subscribe<geometry_msgs::PoseWithCovarianceStamped>("/amcl_pose",3,doPose);
