...
void doPose(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr &p)
{
    ...
    if(ai_car_task == 3)//任务3     S弯起点
    {
        ...
        {
            // send_point( 1.22,-4.3054367687 ,0.734472153866 , 0.678638825293  ,0);//往前一秒，可以顺利拐弯
            send_point( 1.25,-4.2654367687 ,0.734472153866 , 0.678638825293  ,0);//往前一秒，可以顺利拐弯
            ros::Duration du4(1.0);//
            du4.sleep();//
            // ros::param::set("/acc_lim_theta",4.0);
            // ros::param::set("/max_vel_theta",1.5);
            ros::param::set("/acc_lim_theta",5.0);
            ros::param::set("/max_vel_theta",3.0);
            ros::Duration du6(4.08);//
            ...
            flag3 = send_point( 1.00,-4.00 ,0.734472153866 , 0.678638825293  ,0);
            task01_flag = false;
            task02_flag = false;
            task03_flag = true ;
        }
        ...
        if(p->pose.pose.position.x  < -1.1  && p->pose.pose.position.y > -2.2  && p->pose.pose.position.y <-1.3)
        {
            ...
            ros::Duration du9(0.4);
            du9.sleep();//
            ros::param::set("/acc_lim_theta",3.0);
            ros::param::set("/max_vel_theta",0.9);
            start_point = send_point( -0.15 , -0.15 , 0 , 1 ,  1 ); //路径规划回到起点
     ...
     while (ros::ok())
    {
        ...
        else if(ai_car_task == 1)//任务1    装货区
        {
            if(task01_flag == false)
            {
                if(!first_cycle)
                {
                    ros::param::set("/acc_lim_theta",5.0);//开始点给大角加速度
                    ros::param::set("/max_vel_theta",1.2);
                    send_point(  2.15654840234,-3.18160017373,-0.695234006203,0.718783469912 ,0); 
                    ros::Duration du8(1);//大的角加速度行驶1s，使车头可以快速转过来
                    du8.sleep();//
                }
                first_cycle = false;
                ros::param::set("/acc_lim_theta",0.4);//恢复初始的参数
                ros::param::set("/max_vel_theta",1.2);
                flag1 = send_point( 2.15654840234,-3.18160017373,-0.695234006203,0.718783469912 ,1); 
                task01_flag = true;
                task02_flag = false;
                task03_flag = false;
            }
        ...
        else if(ai_car_task == 2)//任务2    卸货区
        {
            ...
            {
                send_point( 2.28654840234,-5.73606816442,-0.695234006203,0.718783469912 ,0);
                ros::Duration du2(1.2);
                du2.sleep();//按照指定的持续时间休眠

                send_point( -2.0, -7.0,0.702149203907  ,0.712029841687,0);
                ros::Duration du5(4);//持续10秒钟,参数是double类型的，以秒为单位
                du5.sleep();//按照指定的持续时间休眠


               flag2 = send_point(-2.10, -6.20627867826, 0.702149203907  ,0.712029841687  ,1 );
                task01_flag = false;
                task02_flag = true;
                task03_flag = false;
            }
