...
L1Controller::L1Controller()
{
  ...
  pn.param("line_final_x", line_final_x, 0.0);//这是直道的目标点
  pn.param("line_final_y", line_final_y, 0.0);


  pn.param("path_choose", path_choose, 1);

  odom_goal_pos.x = line_final_x;
  ...
  timer2 = n_.createTimer(ros::Duration((1.0) / controller_freq), &L1Controller::goalReachingCB, this); // Duration(0.05) -> 20Hz
...
{ //小车到达目标点后
  ...
  if (goal_received)
  {
    ...
    if (car2goal_dist < goalRadius)
    {
      ...
      // ros::param::set("/move_base/planner_frequency",2.0);
      // ros::param::set("/goal_loop/pub_goal_mode",1);
...
void L1Controller::controlLoopCB(const ros::TimerEvent &)
{
  ...
  // cmd_vel.linear.x = -1*baseSpeed;
  // // cmd_vel.angular.z = last_steeringangle;
  // cmd_vel.angular.z = 0;
  ...
  if(goal_received)
  {
    ...
    if (foundForwardPt)
    {
      if (!goal_reached)
      {
        ...
        {
          //only ex once
          baseSpeed = slow_final * baseSpeed + 2;
          stopIdx++;
        ...
        {
          //only ex once
          baseSpeed = fast_final * baseSpeed + 2;
          stopIdx++;
