...
double minlaser = 100000.1;
double mid_laser = 0.26;
double rongrendu = 0.02;
double jiaodu = 5;
...
private:
  ...
  ros::Subscriber amcl_sub,scan_sub;
...
L1Controller::L1Controller()
{
  ...
  pn.param("rongrendu", rongrendu, 0.0);
  pn.param("mid_laser", mid_laser, 0.0);
  pn.param("jiaodu", jiaodu, 0.0);
  ...
  path_sub = n_.subscribe("/path1", 1, &L1Controller::pathCB, this);
  ...
  scan_sub = n_.subscribe("/scan", 10, &L1Controller::lasercb, this);
  
  marker_pub = n_.advertise<visualization_msgs::Marker>("car_path", 10); //发布路径到rviz
  pub_ = n_.advertise<geometry_msgs::Twist>("/cmd_vel_nav", 1);           //发布速度、角速度

  // Timer
  timer3 = n_.createTimer(ros::Duration((5.0) / controller_freq), &L1Controller::odomgoalCB, this);  // Duration(0.05) -> 20Hz
...
{ //里程计回调函数
  ...
  if(amcl_pose.pose.pose.position.y<-6.5 && task_flag == 1)
  {
    // ros::param::set("/trafficlight_enable",0);
    // ros::param::set("/red_traffic",0);
...
void L1Controller::lasercb(const sensor_msgs::LaserScan &msg)
{
  // regions_, e, angle_min, dist_front, diff_e, direction, bool_outer_corner, bool_inner_corner, index, last_kinds_of_wall
  int size,min_index,max_index;
  // size = sizeof(msg.ranges);
  size = 2*PI/msg.angle_increment;
  min_index = 0;
  max_index = size/2;
  minlaser = 1000.2;
  ...
  {
    if (msg.ranges[i] < minlaser && msg.ranges[i] > 0.15)
    {
      minlaser = msg.ranges[i];
    }
    // ROS_INFO("                                                      laser  =   %f",msg.ranges[i]);

  }

  ROS_INFO("                                                      laser  =   %f",minlaser);
}
...
{ //计算前瞻向量
  ...
  forwardPt.x = carPose_pos.x;
  forwardPt.y = carPose_pos.y;
...
void L1Controller::controlLoopCB(const ros::TimerEvent &)
{
  ...
  if(vision_control==0 || vision_control==1){
    if(abs(minlaser-mid_laser)>rongrendu && minlaser<0.35 && task_flag == 1&& amcl_pose.pose.pose.position.x>-0.88){
      if(minlaser-mid_laser<0){
        cmd_vel.angular.z=(mid_laser-minlaser-rongrendu)*jiaodu/57.3*100;
      }else{
        cmd_vel.angular.z=-(minlaser-mid_laser-rongrendu)*jiaodu/57.3*100;
      }
      ROS_INFO("the output is control by laser  =  %f",cmd_vel.angular.z);
    }
    pub_.publish(cmd_vel);
  }
}
